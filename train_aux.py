...
from utils.loss import ComputeLossFCOS, ComputeLossHybrid
...
def train(hyp, opt, device, tb_writer=None):
    ...
    if det_head_mode == 'fcos':
        ...
        compute_loss = ComputeLossHybrid(compute_loss_ota, ComputeLossFCOS(model, hyp=hyp, opt=opt),
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                epoch_positive_count += get_loss_positive_count(compute_loss)
                if rank != -1:
