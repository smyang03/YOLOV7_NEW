...
| 학습 환경 | 최종 개발 착수안 확정 (논문 비교 검토 반영) |
...
Best mAP@0.5: 0.789 (ep045)  │  No improve: 0/20
...
| 파일명 | 내용 | 형식 | 갱신 주기 |
| --- | --- | --- | --- |
| results.csv | 전체 epoch 학습/검증 지표 | CSV | 매 epoch |
| results_per_class.csv | 클래스별 P/R/AP | CSV | 매 epoch |
...
| best.pt | 최고 mAP@0.5 weight | PT | best 갱신 시 |
...
L 모델은 실험 A~D(Loss/Assignment)까지, W6는 A~I 전체 진행한다. Export 검증은 각 구조 변경 직후 중간 체크포인트로 반드시 수행한다.
...
> ※ Export 중간 검증 체크포인트: 실험 A 완료 후 즉시 raw output ONNX → TRT 빌드 검증. 이후 구조 변경(E/F/G/H/I)마다 재검증.
...
Baseline YOLOv7-L/W6 재현

raw output ONNX export

TensorRT FP16 build

C++ NMS 후처리 연결

verify_export.py 작성 및 PyTorch/ONNX/TRT 출력 비교

train.py 통합 + 3-Phase / Rect Finetune

CCTV Aug 조정 + check_aug_visual.py

Hard Negative + Weighted Sampler

Decoupled Head + WIoU + TAL/VFL 공통 적용

W6 P2 Anchor + SCDown 적용

L AUX 옵션 검증

W6 PSA P5 후보 검증

FCOS P2 / GELAN 후순위 검증
...
| Training | A/B 실험 플래그 7개 | ✅ 확정 | 신규 추가 |
...
본 절은 v1.2 최종 개발 착수안을 YOLOv7, WIoU, TOOD/TAL, VarifocalNet, FCOS, YOLOv9, YOLOv10, YOLO LwF 관련 논문 방향과 비교하여 최종 확정/후순위 판단을 정리한다.
...
## 1. Baseline L/W6 학습 결과와 v1.2 최종안 결과를 반드시 동일 validation set에서 비교한다.

## 2. 구조 변경 직후 tools/verify_export.py로 PyTorch/ONNX/TensorRT raw output을 비교한다.

## 3. W6 P2 Anchor/SCDown 적용 후 latency가 목표 범위를 벗어나면 PSA/FCOS/GELAN 실험을 보류한다.

## 4. L 모델은 구조 경량 유지 원칙을 깨지 않는다. L의 성능 개선은 WIoU/TAL/VFL, Aug, Hard Negative 중심으로 처리한다.

## 5. 파인튜닝은 Replay Buffer를 먼저 적용하고, LwF는 forgetting이 남을 때 A/B로 활성화한다.

기밀 — 사내 배포용 / v1.2 최종 개발 요구서
